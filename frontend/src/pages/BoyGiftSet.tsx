...
const BoyGiftSet: React.FC = () => {
  ...
  const handlePrev = () => setCurrent((prev) => (prev === 0 ? boyGiftSet.images.length - 1 : prev - 1));
  const handleNext = () => setCurrent((prev) => (prev === boyGiftSet.images.length - 1 ? 0 : prev + 1));
  ...
  return (
    <Box sx={{ py: { xs: 4, md: 6 } }}>
      <Container maxWidth="lg">
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(2, 1fr)' }, gap: 4 }}>
          <Box>
            ...
            <Box sx={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', mb: 2 }}>
              ...
              >
                <img
                  src={boyGiftSet.images[current]}
                  alt={boyGiftSet.title}
                  style={{
                    width: '100%',
                    height: '100%',
                    maxHeight: 500,
            ...
            <Box sx={{ display: 'flex', gap: 2, mt: 2, justifyContent: 'center' }}>
              {boyGiftSet.images.map((img, idx) => (
                <Box
                  key={img}
                  component="img"
                  src={img}
                  alt={boyGiftSet.title + ' ' + (idx + 1)}
                  sx={{
                    width: 60,
                    height: 60,
                    objectFit: 'cover',
            ...
            <Box sx={{ mb: 4 }}>
              ...
              <Box sx={{ display: 'flex', gap: 2, mb: 4 }}>
                <AddToCartButton
                  item={{
                    id: boyGiftSet.id,
                    title: boyGiftSet.title,
                    price: boyGiftSet.price,
                    image: boyGiftSet.images[0],
                  }}
                  variant="contained"
                  size="large"
                />
                <Button
                  ...
                  onClick={async () => {
                    const shareData = { title: boyGiftSet.title, text: boyGiftSet.description, url: window.location.href };
                    try {
                      if (navigator.share) {
        ...
        {/* Модальное окно для увеличения фото с галереей */}
