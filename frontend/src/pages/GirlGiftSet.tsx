...
const GirlGiftSet: React.FC = () => {
  ...
  const handlePrev = () => setCurrent((prev) => (prev === 0 ? girlGiftSet.images.length - 1 : prev - 1));
  const handleNext = () => setCurrent((prev) => (prev === girlGiftSet.images.length - 1 ? 0 : prev + 1));
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(2, 1fr)' }, gap: 4 }}>
          <Box>
            ...
            <Box sx={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', mb: 2 }}>
              ...
              >
                <img
                  src={girlGiftSet.images[current]}
            ...
            <Box sx={{ display: 'flex', gap: 2, mt: 2, justifyContent: 'center' }}>
              {girlGiftSet.images.map((img, idx) => (
                <Box
                  key={img}
                  component="img"
                  src={img}
                  alt={girlGiftSet.title + ' ' + (idx + 1)}
                  sx={{
                    width: 60,
                    height: 60,
                    objectFit: 'cover',
            ...
            <Box sx={{ mb: 4 }}>
              ...
              <Box sx={{ display: 'flex', gap: 2, mb: 4 }}>
                ...
                <Button
                  ...
                  onClick={async () => {
                    const shareData = { title: girlGiftSet.title, text: girlGiftSet.description, url: window.location.href };
                    try {
                      if (navigator.share) {
